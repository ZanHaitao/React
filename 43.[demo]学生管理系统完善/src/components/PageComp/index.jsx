...
export default class index extends Component {
  ...
  getMinPageNumber() {
    ...
    if (flag !== 0 && flag < middle) {
      return this.props.current - middle - flag < 1
        ? 1
        : this.props.current - middle - flag;
    } else if (flag === 0) {
      return this.props.current - this.props.panelPage + 1 < 1
        ? 1
        : this.props.current - this.props.panelPage + 1;
    }
  ...
  getMaxPageNumber(min) {
    return min + this.props.panelPage - 1 > this.pageNumber
      ? this.pageNumber
      : min + this.props.panelPage - 1;
  }
  ...
  getPage() {
    ...
    numbers.push(
      <div
        ...
        className={this.props.current === 1 ? "page-btn active" : "page-btn"}
      >
        1
      </div>
    );
    if (minPage > 2) {
      numbers.push(
        <div key={999} className="page-tip">
          ...
        </div>
      );
    }
    for (let i = minPage; i <= maxPage; i++) {
      ...
      numbers.push(
        <div
          ...
          className={this.props.current === i ? "page-btn active" : "page-btn"}
        >
          {i}
        </div>
      );
    ...
    if (maxPage < this.pageNumber - 1) {
      numbers.push(
        <div key={9999} className="page-tip">
          ...
        </div>
      );
    }
    if (this.pageNumber !== 1) {
      numbers.push(
        <div
          ...
          className={
            this.props.current === this.pageNumber
              ? "page-btn active"
              : "page-btn"
          }
        >
